...
-- if-then-else
reverseList :: [a] -> [a]
reverseList l = if length l > 1 then reverseList (tail l) ++ [(head l)] else l
...
reverseList4 :: [a] -> [a]
reverseList4 l
    | length l > 1 = reverseList (tail l) ++ [(head l)]
    | otherwise = l

-- foldl
reverseList5 :: [a] -> [a]
reverseList5 l = foldl (\x y -> y:x) [] l

-- foldr
reverseList6 :: [a] -> [a]
reverseList6 l = foldr (\x y -> y ++ [x]) [] l
...
numToBase3 n b
    | n /= 0 = (numToBase (quot n b) b) ++ [(mod n b)]
    | otherwise = []
